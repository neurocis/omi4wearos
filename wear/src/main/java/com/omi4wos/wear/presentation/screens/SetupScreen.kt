...
/**
 * Setup screen — displays a QR code pointing to the watch's local web server.
 *
 * Flow:
 *   1. Watch starts [SetupServer] on port 8080.
 *   2. QR code shows `http://<watch-wifi-ip>:8080`.
 *   3. User scans on any phone → fills the form (API key + email + password).
 *   4. On success the screen shows a confirmation and the server shuts down.
 */
@Composable
fun SetupScreen(onDone: () -> Unit = {}) {
    ...
    DisposableEffect(Unit) {
        ...
        if (ip == null) {
            ...
            val url = "http://$ip:${SetupServer.PORT}"
            watchUrl = url
    ...
    Box(
        ...
        when (state) {
            ...
            SetupState.SUCCESS -> {
                val configured = remember { StandaloneOmiConfig(context).load().isConfigured }
                Column(
                    modifier = Modifier.padding(16.dp),
                    horizontalAlignment = Alignment.CenterHorizontally,
                    verticalArrangement = Arrangement.Center
                    ...
                    Text(
                        text = if (configured) "✓ Connected!" else "Setup complete",
                        fontSize = 14.sp,
